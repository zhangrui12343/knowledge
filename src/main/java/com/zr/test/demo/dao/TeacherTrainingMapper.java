...
public interface TeacherTrainingMapper extends BaseMapper<TeacherTraining> {
    @Select("select id,name,img,description from teacher_training where type=#{type} and status=1  order by time desc limit 3")
    List<OtherCourseWebVO> selectLimit3(@Param("type") Long type);

    @Select("<script>" +
            "select a.id,a.name,a.img,a.count from teacher_training as a " +
            "left join teacher_traning_tag_relation as b on a.id=b.after_course_id " +
            "left join teacher_traning_category_relation as c on a.id=c.after_course_id " +
            " where a.status=1  " +
    ...
    @Select("update teacher_training set count= count +1 where id =#{id}")
