...
public interface AfterCourseMapper extends BaseMapper<AfterCourse> {
    ...
    @Select("select id,name,img,description from after_course where type=#{type} and status=1 order by time desc limit 3")
    List<OtherCourseWebVO> selectLimit3(@Param("type") Long type);

    @Select("<script>" +
            "select a.id,a.name,a.img,a.count from after_course as a " +
            "left join after_course_tag_relation as b on a.id=b.after_course_id " +
            "left join after_course_category_relation as c on a.id=c.after_course_id " +
            "where a.status=1 " +
    ...
    @Select("update after_course set count= count +1 where id =#{id}")
