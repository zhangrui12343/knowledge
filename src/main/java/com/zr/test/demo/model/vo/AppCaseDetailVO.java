package com.zr.test.demo.model.vo;

import com.zr.test.demo.model.entity.AppCase;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.experimental.Accessors;

import java.io.Serializable;
import java.util.List;
import java.util.Map;

/**
 * <p>
 *
 * </p>
 *
 * @author zr
 * @since 2022-05-14
 */
@Data
@EqualsAndHashCode(callSuper = false)
@Accessors(chain = true)
@ApiModel(value = "信课融合案例出参", description = "")
public class AppCaseDetailVO implements Serializable {
    private Long id;
    @ApiModelProperty(value = "软件名称", example = "1213")
    private String appName;
    @ApiModelProperty(value = "软件id", example = "1213")
    private Long appId;
    @ApiModelProperty(value = "案例名称", example = "1213")
    private String caseName;
    @ApiModelProperty(value = "视频文件路径", example = "1213")
    private String videoPath;
    @ApiModelProperty(value = "视频文件id", example = "1213")
    private Long video;
    @ApiModelProperty(value = "软件介绍", example = "1213")
    private String appIntroduction;
    @ApiModelProperty(value = "功能介绍", example = "1213")
    private String feature;
    @ApiModelProperty(value = "案例更新时间", example = "1213")
    private String time;


    @ApiModelProperty(value = "标签", example = "1213")
    private List<String> tags;

    @ApiModelProperty(value = "软件推荐")
    private List<AppAndCaseVO> apps;

    @ApiModelProperty(value = "案例推荐")
    private List<AppCase> cases;

}
