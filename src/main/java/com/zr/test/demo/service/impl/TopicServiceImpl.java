...
public class TopicServiceImpl extends ServiceImpl<TopicMapper, Topic> implements ITopicService {
    ...
    private final TopicTagRelationServiceImpl tagRelationService;
    private final TopicCategoryRelationServiceImpl categoryRelationService;
    private final TopicTagRelationBiz tagRelationBiz;
    private final TopicCategoryRelationBiz categoryRelationBiz;
    private final FirstCategoryServiceImpl firstCategoryService;
    private final FileRouterBiz fileRouterService;

    public TopicServiceImpl(TopicTagRelationServiceImpl tagRelationService,
            TopicCategoryRelationServiceImpl categoryRelationService,
            TopicTagRelationBiz tagRelationBiz,
            TopicCategoryRelationBiz categoryRelationBiz,
            FirstCategoryServiceImpl firstCategoryService,
            FileRouterBiz fileRouterService) {
    ...
    public Result<Object> add(OtherCourseDTO dto, HttpServletRequest request) {
        ...
        if (i == 1) {
            dto.getTags().forEach(id -> tagRelationService.getBaseMapper().insert(new TopicTagRelation(vo.getId(), id)));
            dto.getCategories().forEach(id -> categoryRelationService.getBaseMapper().insert(new TopicCategoryRelation(vo.getId(), id)));
        }
    ...
    public Result<PageInfo<OtherCourseVO>> query(OtherCourseQueryDTO dto, HttpServletRequest request) {
        ...
        Page<Topic> pages = PageHelper.startPage(dto.getPage(), dto.getPageSize()).
                doSelectPage(() -> this.baseMapper.selectList(queryWrapper));
        List<Topic> list = pages.getResult();
    ...
    public Result<Object> update(OtherCourseDTO dto, HttpServletRequest request) {
        ...
        QueryWrapper<TopicCategoryRelation> queryWrapper2 = new QueryWrapper<>();
        queryWrapper2.eq("after_course_id", dto.getId());
        categoryRelationService.getBaseMapper().delete(queryWrapper2);
        dto.getCategories().forEach(c -> categoryRelationService.getBaseMapper().insert(new TopicCategoryRelation(dto.getId(), c)));
        QueryWrapper<TopicTagRelation> queryWrapper1 = new QueryWrapper<>();
        queryWrapper1.eq("after_course_id", dto.getId());
        tagRelationService.getBaseMapper().delete(queryWrapper1);
        dto.getTags().forEach(c -> tagRelationService.getBaseMapper().insert(new TopicTagRelation(dto.getId(), c)));
    ...
    public Result<WebAfterDetailVO> detail(Long id) {
        Topic entity = this.baseMapper.selectById(id);
        if (entity == null) {
            return Result.success(null);
        }
