...
public enum  ErrorCode {
    ...
    TERID_EXIST(407,"终端编号已存在"),
    CARLIENCE_EXIST(408,"车牌号已存在"),
    NO_TERID(409,"该车辆不存在"),
    SOME_CAR_ERROR(410,"部分车辆同步失败"),
    SOME_GROUP_ERROR(411,"部分车组同步失败"),
    EXEC_SQL_ERROR(412,"sql执行失败"),
    SENDREDIS_ERROR(413,"发送数据到redis失败"),
    SENDREDIS_FREQUENT(414,"发送数据到redis失败"),
    SENDTODEVICE_TIMEOUT(415,"发送数据到设备超时"),
    KEY_EXIST(416,"key已存在"),
    EVIDENCE_STRATEGY_NO_EXIST(417,"证据策略不存在"),
    NEW_MAX_DRIVER_CNT(418,"下发人脸司机达到最大数量1000"),
    TIMENOTINDAY_ERROR(419,"时间没在一天，限制查询使用"),

    EVIDENCE_LOCK_EXITS(420,"有用户正在处理"),
    EVIDENCE_UNLOCK_AUTH(421,"无操作证据权限"),
    EVIDENCE_SESIONID_NOT_EXIST(422,"证据会话sessionid不存在"),
    EVIDENCE_FILES_NOT_EXIST(423,"证据文件数据Mongo未找到"),
